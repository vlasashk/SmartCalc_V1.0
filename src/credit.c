...
void annuityCalc(CreditInput input, CreditOutput *output) {
  ...
  double remain_loan = input.total_loan;
  double coeff = monthly_rate / (1 - pow(1 + monthly_rate, input.term * -1));
  output->max_monthly = input.total_loan * coeff;
...
void diffCalc(CreditInput input, CreditOutput *output) {
  ...
  for (int i = 0; i < input.term; i++) {
    ...
    remain_loan -= round((input.total_loan / input.term) * 100.0f) / 100.0f;
...
CreditInput initCreditInput() {
  CreditInput input = {0};
  input.annuity = 0;
  input.total_loan = 12000000.0f;
  input.term = 24;
  input.yearly = 0;
  input.interest = 3.5f;
  input.curr_year = 2023;
  input.curr_month = 4;
  return input;
}
...
CreditOutput creditCalculator() {
  CreditInput input = initCreditInput();
