...
QVector<double> MainWindow::calculateYValues(QVector<double> xValues, Stack *input) {
...
void MainWindow::plotGraph(QVector<double> xValues, QVector<double> yValues) {
    ...
    for (int i = 0; i < yValues.size(); i++) {
        if ((qIsInf(yValues[i]) || qIsNaN(yValues[i]))) {
            ...
            if (!qIsNaN(previous)){
                xSegments.append(xValues.mid(currentSegmentStartIndex, i - currentSegmentStartIndex));
                ySegments.append(yValues.mid(currentSegmentStartIndex, i - currentSegmentStartIndex));
