...
void MainWindow::basicInputLineUI() {
    ...
    if (inputString == "0" && (simpleOperationCheck(buttonText) || buttonText == "-") && buttonText != ".") {
        inputString = buttonText;
    } else {
        inputString.append(buttonText);
    }
    if (switchField || (!switchField && inputLimitationX(buttonText))) {
...
bool MainWindow::simpleOperationCheck(QString button) {
    bool res = true;
    if (button == "^" || button == "/"|| button == "*" || button == "-" || button == "+" || button == "mod") {
        res = false;
    }
    return res;
}
